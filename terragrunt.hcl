remote_state {
  backend = "s3"
  config = {
    bucket         = "natura.banking.prd-sa.foundation.terragrunt.v1"
    key            = "financial-services/${path_relative_to_include()}/terraform.tfstate"
    region         = "sa-east-1"
    encrypt        = true
...
inputs = {
  tfstate_global_bucket        = "natura.banking.prd-sa.foundation.terragrunt.v1"
  tfstate_global_bucket_region = "sa-east-1"
}
